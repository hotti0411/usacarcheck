...
// Drop obvious heavy/commercial vehicles (semis, cargo vans, chassis cabs): GVWR > 14k / not
// personal-use, so not deduction-eligible and no real search value. Conservative name matching.
const COMMERCIAL = /(fuso|canter|sprinter|metris|promaster|chassis|cutaway|incomplete|low cab forward|medium duty|\bVNL\b|\bVNR\b|\bVHD\b|\bVAH\b|\bVNX\b)/i;
function isHeavyCommercial(v) {
  if (COMMERCIAL.test(v.model)) return true;
  // Volvo: keep only passenger nameplates (XC/S/V/C/E + digit); drop the truck lineup (VNL, VNR, VHD…).
  if (v.makeSlug === "volvo" && !/^(xc\d|s\d\d|v\d\d|c\d\d|e[xc]\d)/i.test(v.model)) return true;
  return false;
}

export const vehicles = allVehicles.filter((v) => !isHeavyCommercial(v));
